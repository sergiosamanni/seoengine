...
import {
  ...
  PenTool, ChevronRight, Eye, Sparkles, History, ChevronDown, Trash2, ImagePlus, X
} from 'lucide-react';
...
// ========== ARTICLE HISTORY ==========
const ArticleHistory = ({ effectiveClientId, getAuthHeaders }) => {
  ...
  const fetchArticles = async () => {
    if (articles.length > 0) { setOpen(!open); return; }
    setLoading(true);
    setOpen(true);
    try {
                      ...
                      {expandedId === a.id && expandedContent && (
                        <div className="mt-3 p-3 bg-slate-50 rounded-lg border border-slate-200 text-sm">
                          ...
                          <div className="prose prose-sm max-w-none text-slate-700 max-h-[300px] overflow-y-auto"
                            dangerouslySetInnerHTML={{ __html: expandedContent.contenuto_html?.slice(0, 2000) || '<p>Nessun contenuto</p>' }} />
                          {expandedContent.contenuto_html?.length > 2000 && <p className="text-xs text-slate-400 mt-2">... contenuto troncato</p>}
                        </div>
                      )}
                    </div>
                  ))}
