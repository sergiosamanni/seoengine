    ...
    const handleSingleGenerate = async (typeOverride = null) => {
        ...
        try {
            ...
            const payload = {
                ...
                publish_to_wordpress: publishToWp,
                content_type: state.contentType || (genMode === 'pillar' ? 'pillar_page' : 'articolo'),
                objective: 'informazionale',
                silo_context: state.siloContext || undefined,
                image_ids: (imageSource !== 'ai' && state.selectedImages.length > 0) ? state.selectedImages.map(img => img.id) : (adminUploadedImages.length > 0 ? adminUploadedImages.map(img => img.id) : undefined),
