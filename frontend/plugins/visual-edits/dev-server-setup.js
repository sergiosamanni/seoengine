...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all seoengine.it subdomains
      ...
      try {
        ...
        for (const change of variableEditChanges) {
          ...
          if (result.success) {
            ...
            try {
              execSync(`git -c user.name="visual-edit" -c user.email="admin@seoengine.it" add "${result.file}"`);
              execSync(`git -c user.name="visual-edit" -c user.email="admin@seoengine.it" commit -m "visual_edit_variable_${timestamp}"`);
            } catch (gitError) {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="admin@seoengine.it" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="admin@seoengine.it" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
