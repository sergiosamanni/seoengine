...
async def generate_topic_image(body: dict, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        result = await generate_image_with_fallback(crafted_prompt, user_id, openai_key=llm_key)
...
async def serp_images(request: dict, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Format the results to only include the necessary fields
        formatted_results = []
        for r in results:
            w = r.get("width", 800)
            h = r.get("height", 600)
            # Estimate KB based on dimensions for a typical JPEG
            kb_est = int((w * h * 0.0002))
            ...
            formatted_results.append({
                ...
                "width": w,
                "height": h,
                "weight_kb": max(15, kb_est)
            })
        return {"keyword": keyword, "results": formatted_results}
    except Exception as e:
        import logging
            ...
            if not content:
                ...
                if generate_cover:
                    try:
                        from agents.image import ImageAgent
                        from helpers import generate_image_with_fallback
                        # Use same logic as manual generator but automated
                        ...
                        img_res = await generate_image_with_fallback(crafted_img_prompt, "admin", openai_key=llm_config.get("api_key"))
                        if img_res and img_res.get("id"):
                            featured_image_ids.append(img_res["id"])
                            result["image_id"] = img_res["id"]
                            logger.info(f"Auto-generated image for batch article '{titolo}': {img_res['id']}")
