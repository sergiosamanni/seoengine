...
async def unrent_cleanup(drafts_only: bool = False, current_user: dict = Depends(require_admin)):
    ...
    If drafts_only=True, deletes articles with status 'draft' or 'failed'.
    ...
    if drafts_only:
        # Delete BOTH drafts and failed (the ones in the screenshot)
        res = await db.articles.delete_many({
            "client_id": cid,
            "stato": {"$in": ["draft", "failed"]}
        })
        msg = f"Successfully deleted {res.deleted_count} draft/failed articles for Unrent."
    else:
